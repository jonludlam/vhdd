...
let rec master_retry_loop context allowed_errors bad_errors f metadata host =
  let is_local = host.h_uuid = Global.get_host_uuid () in
  if not is_local && host.h_ip = None then failwith "No IP";
  try
    ...
    let remote_rpc call = 
      ...
      let result = Int_rpc.wrap_rpc (Vhdrpc.remote_rpc context.c_task_id (match host.h_ip with Some x -> x | None -> "x") host.h_port) call in
      Tracelog.append context (Tracelog.Master_to_slave_response (rpc_of_intrpc_response_wrapper result)) None;
      result
    in
    let local_rpc call =
      Tracelog.append context (Tracelog.Master_to_slave_call (rpc_of_intrpc call)) None;
      let result = Int_rpc.wrap_rpc (!(Vhdrpc.local_rpc) context.c_task_id) call in
      Tracelog.append context (Tracelog.Master_to_slave_response (rpc_of_intrpc_response_wrapper result)) None;
      result
    in
    if is_local
